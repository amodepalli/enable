...
possible_explanatory="Stature Weight Shoulder_Breadth Shoulder_Elbow_Length Upper_Arm_Circumference Elbow_Hand_Length Forearm_Circumference Hand_Length Hand_Breadth".split()
...
rsquared=[0]*30
estimate=[0]*30
new_explanatory_var=explanatory_var
stop=1;
sample=[1,1000,300]
new_sample=[]
#this is to output the params as a big variable that we can work with in predict.py, which predicts the hand size given some user features


def generate_regression_data():
	global stop,new_sample,sample,explanatory_var
	#print "format: combined, boys, girls"
	for word in response_var:
		to_analyze = "{} ~ {}".format(word, explanatory_var)
		to_write = "{}".format(word)
		write_to_file(linear_file_both, data, to_analyze, to_write, "linear", word, explanatory_var,possible_explanatory,rsquared,response_var.index(word),sample)
		...
		print "--------------------------------------------------------"
	explanatory_var=new_explanatory_var
...
def write_to_file(file, data, to_analyze, to_write, type, response_var, explanatory_var,possible_explanatory,rsquared,index,sample):
	result = []
	cur_estimate=0
	global new_sample,stop,new_explanatory_var
	...
	print sample
	if result.rsquared>rsquared[index] and result.rsquared<1:
		rsquared[index]=result.rsquared
		
		for x in range(len(sample)):
			cur_estimate=cur_estimate+sample[x]*result.params[x]
		estimate[index]=cur_estimate
		print estimate[index]
		print result.rsquared
	
	if result.rsquared>=0.94 and result.rsquared<1 and response_var in possible_explanatory:
		cur_stop=0
		new_explanatory_var= new_explanatory_var +" + "+ response_var
		print new_explanatory_var
		new_sample.extend([cur_estimate])
		stop=0
